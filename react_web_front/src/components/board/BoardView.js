          ...
          <div className="board-view-preview">
            <table className="tbl">
              <tbody>
                ...
                <tr>
                  <th style={{ widows: "20%" }}>작성자</th>
                  <td style={{ width: "30" }}>{board.boardWriter}</td>
                  <th style={{ widows: "20%" }}>작성일</th>
...
const FileItem = (props) => {
  ...
  const filedown = () => {
    axios
      ...
      .then((res) => {
        console.log(res);
        //서버에서 받은 데이터를 javascript의 Blob객체로 변환
        const blob = new Blob([res.data]);
        //blob데이터를 이용해서 데이터 객체 url생성(다운로드 할 수 있는 링크)
